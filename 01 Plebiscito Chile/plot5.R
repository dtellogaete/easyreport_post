...
votes <- data %>%
            filter(year == 1925) %>%
            select(-name_c, -name_a, -name_b, -year, -votantes, -nulo, -blanco)
votes <- data.frame(t(votes))
votes$votes <- as.integer(votes$votes)
colnames(votes) <- "votes"
votes$label <- c("Roja", "Azul", "Blanca")
...
ggplot(votes, aes(reorder(label, votes/1e3), votes/1e3, 
                       fill=label))+
  geom_col()+
  coord_flip()+
  labs(y = "Votos (miles)", x = NULL)+
  scale_fill_manual(breaks = c("Si", "No", "Nulo"), 
                    values=c("blue","white", "#FF0033"))+
